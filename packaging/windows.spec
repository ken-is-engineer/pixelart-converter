# PyInstaller spec for an onedir Windows folder (COLLECT, no onefile).
#
# Entry is the same as `python -m pixelart_converter`: src/pixelart_converter/__main__.py
# with `src` on pathex. Output folder is dist/pixelart-converter/.
#
...
# have Addons installed does not pull them into the bundle.
#
# ffmpeg: copied into datas as vendor/ffmpeg/windows/ffmpeg.exe when present so
# conversion.binary.resolve_ffmpeg can find it under sys._MEIPASS. Missing
# binary prints a warning; scripts/build_windows.ps1 refuses to build.
